...
def package_lambda_code(project, logger):
    ...
    sources = os.path.join(project.expand_path("$dir_source_main_python"))
    copy_dir_content_to_ziproot(archive, sources)
    scripts = os.path.join(project.expand_path("$dir_source_main_scripts"))
